        ...
        <tr v-for="item in items" :key="item.id">
            <td v-for="(header, index) in headers" :key="index">
                <template v-if="header.type === 'img'">
                    <img alt="" style="max-height: 90px;" :src="item[header.code]"/>111
                </template>
                <template v-else-if="header.type === 'url'">
                    <a :href="item[header.url]">{{item[header.code]}}</a>222
                </template>
                <template v-else-if="header.type === 'badge'">
                    <span class="badge" :class="item[header.code].class">{{item[header.code].title}}</span>333
                </template>
                <template v-else>
                    {{item[header.code]}}444
                </template>
